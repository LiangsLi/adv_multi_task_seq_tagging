...
parser.add_argument('--num_corpus', default=21, type=int)
...
# checkpoint
parser.add_argument('--use_given_ckp', default=True, type=bool)
parser.add_argument('--pre_trained_ckp', default='../20_ckp/multi_task_20_1809061003/checkpoints', type=str)
# show final result
parser.add_argument('--show_final_result', default=False, type=bool)
parser.add_argument('--use_one_shared_ckp', default=9, type=int)
...
def Load_pkbs(use_shared_ckp_idx=-1):
    ...
    if use_shared_ckp_idx != -1:
        ...
        logger.info("USE given shared ckp,Task idx: "+str(use_shared_ckp_idx))
        ...
        raise RuntimeError("transform train must give a shared ckp ")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        for var in tf.trainable_variables():
            if var.name.split('/')[0] in ['shared', 'domain']:
        ...
        if FLAGS.private_train:
            ...
            best_accuary = [0] * FLAGS.num_corpus
            best_step_private = [0] * FLAGS.num_corpus
            ...
            for i in range(FLAGS.num_epochs_private):
                ...
                if stop is False:
                    for j in range(21, FLAGS.num_corpus + 1):
                        ...
                        if current_step % FLAGS.evaluate_every == 0:
                            ...
                            if best_accuary[j - 1] < new_f:
                                ...
                                # 超过200步都没有取得更好的结果
                                logger.info(
                                    "Private train : Task {} early stop in step:{},best F1 is {}".format(j,
                                                                                                         current_step,
                                                                                                         best_accuary[
                                                                                                             j - 1]))
